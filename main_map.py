...
def load_full_data():
    try:
        ...
        df_prov_final = df_latest[df_latest['기초지역'] == '전체']
        return df, df_latest, df_prov_final
    except Exception as e:
        st.error(f"데이터 연결 오류: {e}")
        return None, None, None

df_raw, df_latest, df_prov = load_full_data()
...
# [모드 1] 현행 판세 분석 (상세 그래프 복구)
if app_mode == "현행 판세 분석":
    st.subheader("📈 실시간 전국 광역 시·도별 판세")
    
    # 지역 선택 버튼 (상단 배치)
    st.write("상세 정보를 보려는 지역을 선택하세요:")
    cols = st.columns(6)
    all_regs = sorted(HEX_MAP.keys())
    for i, reg in enumerate(all_regs):
        with cols[i % 6]:
            if st.button(f"{reg}", key=f"prov_btn_{reg}", use_container_width=True,
    ...
    # 해당 지역의 광역(전체) 데이터 필터링
    region_data = df_prov[df_prov['지역'] == sel_reg]
    
    if not region_data.empty:
        # 1. 바 차트 출력
        cmap = {'더불어민주당': '#004EA2', '국민의힘': '#E61E2B', '민주당': '#004EA2', '국힘': '#E61E2B'}
        fig_prov_bar = px.bar(region_data, x='후보', y='지지율', color='정당', 
                              text=region_data['지지율'].apply(lambda x: f"{x:.1f}%"),
                              title=f"[{sel_reg}] 후보별 지지율 현황", color_discrete_map=cmap)
        st.plotly_chart(fig_prov_bar, use_container_width=True)
        
        # 2. 데이터 표 출력
        st.write("### 📋 광역 지지율 상세 데이터")
        st.dataframe(region_data[['후보', '정당', '지지율']].sort_values('지지율', ascending=False), 
                     hide_index=True, use_container_width=True)
    else:
        st.warning(f"🔔 {sel_reg} 지역의 광역 데이터가 아직 업데이트되지 않았습니다.")
    ...
    if is_valid:
        ...
        for i, reg in enumerate(all_regs):
            with cols[i % 6]:
                ...
                if st.button(f"{prefix}{reg}", key=f"muni_btn_{reg}", use_container_width=True,
                             type="primary" if st.session_state['selected_region'] == reg else "secondary"):
                    st.session_state['selected_region'] = reg
                    st.rerun() 
        ...
        if not sub_df.empty:
            st.markdown(f"### 🚩 {sel_reg} 시군구 상세 분석 결과")
            cmap = {'더불어민주당': '#004EA2', '국민의힘': '#E61E2B', '민주당': '#004EA2', '국힘': '#E61E2B'}
...
# [모드 4] 가상 시뮬레이터
    ...
    st.subheader("🎛️ 전국 단위 지지율 시뮬레이션")
    if is_valid:
        col_s1, col_s2 = st.columns(2)
        adj_minju = col_s1.slider("🔵 민주당 조정 (%p)", -15.0, 15.0, 0.0, 0.5)
        adj_gukhim = col_s2.slider("🔴 국힘 조정 (%p)", -15.0, 15.0, 0.0, 0.5)
        df_sim = df_prov.copy()
