...
BRAND_INDIGO = "#1A237E" 
GRAY_LIGHT = "#F5F5F5"   
SEL_FILL = "#E3F2FD"     # 선택된 지역 배경
SEL_LINE = "#1565C0"     # 선택된 지역 테두리 (강조용 파랑)

# [수정] CSS 주입: 빨간색 버튼 문제를 해결하고 디자인을 깔끔하게 만듭니다.
st.markdown(f"""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700;900&display=swap');
    html, body, [class*="css"] {{ font-family: 'Noto Sans KR', sans-serif; }}
    
    /* [핵심] 빨간색 버튼을 브랜드 남색으로 강제 변경 */
...
def final_visual_map_engine(df, title_text, highlight_region="", mode="normal", active_regions=None):
    ...
    for region, (col, row) in HEX_MAP.items():
        ...
        # 0순위: 선택된 지역 (경계선을 선명하고 깨끗하게)
        if region == highlight_region:
            f_color = SEL_FILL   
            l_color = SEL_LINE  
            l_width = 5 # 두께를 10에서 5로 줄여 겹침 현상 방지
            t_color = BRAND_INDIGO
            h_text = f"<b>{region} (분석 중)</b>"
        
        # 1단계: 시군구 현황 모드
        elif mode == "status":
            is_active = region in active_regions
            f_color = BRAND_INDIGO if is_active else GRAY_LIGHT
            t_color = "white" if is_active else "#ADB5BD"
            l_color = "white"
            h_text += f"<br>{'데이터 있음' if is_active else '대기 중'}"
        
        # 2단계: 판세 결과 모드 (농도 계산 포함)
        else:
            if df is not None and not df.empty:
                ...
                if not r_all.empty:
                    ...
                    alpha = max(0.2, min(gap / 25.0, 1.0)) # 격차에 따른 농도
                    
                    if '민주' in p_orig: f_color = f'rgba(0, 78, 162, {alpha})'; t_color = 'white' if alpha > 0.4 else BRAND_INDIGO
                    elif '국힘' in p_orig: f_color = f'rgba(230, 30, 43, {alpha})'; t_color = 'white' if alpha > 0.4 else BRAND_INDIGO
...
if app_mode == "현행 판세 분석":
    ...
    if is_valid:
        ...
        with col_s1: adj_minju = st.slider("🔵 민주당 조정", -15.0, 15.0, 0.0, 0.5)
        with col_s2: adj_gukhim = st.slider("🔴 국힘 조정", -15.0, 15.0, 0.0, 0.5)
        ...
        st.plotly_chart(final_visual_map_engine(df_sim, "시뮬레이션 결과"), use_container_width=True)
