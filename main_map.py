...
BRAND_INDIGO = "#1A237E" 
GRAY_LIGHT = "#F5F5F5"   
SEL_FILL = "#E3F2FD"     # 클릭 시 배경 (연한 파스텔 블루)
SEL_LINE = "#1565C0"     # 클릭 시 테두리 (진한 파랑)
...
# [V6.4] 시군구 모드 전용 지도 함수 (빨간색 로직이 아예 없음)
def draw_status_map(title_text, active_regions, selected_region):
    fig = go.Figure()
    for region, (col, row) in HEX_MAP.items():
        cx, cy, x_coords, y_coords = get_hexagon_path(col, row)
        
        # 색상 로직: 1순위 선택됨(하늘색) > 2순위 데이터있음(남색) > 3순위 대기(회색)
        if region == selected_region:
            f_color = SEL_FILL
            l_color = SEL_LINE
            l_width = 10
            t_color = BRAND_INDIGO
            h_text = f"<b>{region} (현재 분석 중)</b>"
        elif region in active_regions:
            f_color = BRAND_INDIGO
            l_color = "white"
            l_width = 2
            t_color = "white"
            h_text = f"<b>{region} (데이터 분석 가능)</b>"
        else:
            f_color = GRAY_LIGHT
            l_color = "white"
            l_width = 2
            t_color = "#9E9E9E"
            h_text = f"<b>{region} (업데이트 대기)</b>"
...
# [V6.4] 현행 판세 전용 지도 함수
def draw_result_map(df, title_text):
    fig = go.Figure()
    for region, (col, row) in HEX_MAP.items():
        cx, cy, x_coords, y_coords = get_hexagon_path(col, row)
        color, t_color = '#F0F2F6', BRAND_INDIGO
        h_text = f"<b>{region}</b>"
        ...
        if df is not None and not df.empty:
            ...
            if not r_all.empty:
                ...
                if '민주' in p_orig:
                    ...
                    t_color = 'white' if alpha >= 0.5 else BRAND_INDIGO
        ...
        fig.add_trace(go.Scatter(x=x_coords, y=y_coords, fill='toself', fillcolor=color, mode='lines', 
                                 line=dict(color="white", width=2), name=region, text=h_text, hoverinfo='text'))
        fig.add_trace(go.Scatter(x=[cx], y=[cy], mode='text', text=[f"<b>{region}</b>"], 
                                 textfont=dict(color=t_color, size=15, family="Noto Sans KR"), hoverinfo='skip'))
...
with st.sidebar:
    ...
    st.markdown("<div style='text-align:center; color:red; font-weight:bold; font-size:16px;'>V6.4 (Color Fix)</div>", unsafe_allow_html=True)
...
if app_mode == "시군구 판세 분석":
    ...
    if is_valid:
        ...
        # [중요] 'draw_status_map'은 지지율 데이터를 아예 받지 않으므로 빨간색이 절대 나올 수 없습니다.
        st.plotly_chart(draw_status_map(f"🔍 {sel_reg} 상세 분석 중", active_regions, sel_reg), use_container_width=True)
        ...
        if not sub_df.empty:
            ...
            fig_sub = px.bar(sub_df, x='기초지역', y='지지율', color='정당', text=sub_df['지지율'].apply(lambda x: f"{x:.1f}%"),
                             title=f"[{sel_reg}] 기초자치단체별 지지율 현황", barmode='group', color_discrete_map=cmap)
...
# (이하 현행 판세 및 다른 모드는 'draw_result_map' 사용)
elif app_mode == "현행 판세 분석":
    df_prov = df_current_latest[df_current_latest['기초지역'] == '전체'] if is_valid else None
    st.plotly_chart(draw_result_map(df_prov, "전국 광역 시·도별 판세 현황"), use_container_width=True)

elif app_mode == "2025 대선 비교 분석":
    col1, col2 = st.columns(2)
    df_prov = df_current_latest[df_current_latest['기초지역'] == '전체'] if is_valid else None
    with col1: st.plotly_chart(draw_result_map(df_2025, "🗳️ 2025년 대선 결과"), use_container_width=True)
    with col2: st.plotly_chart(draw_result_map(df_prov, "📈 현재 판세 실시간 데이터"), use_container_width=True)
    ...
    if not is_valid: st.warning("데이터를 불러오는 중입니다...")
        ...
        st.plotly_chart(draw_result_map(df_sim, "시뮬레이션 결과"), use_container_width=True)
