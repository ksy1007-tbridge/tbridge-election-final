...
h_text = f"<b>{region}</b>"
    ...
    if region == highlight_region:
        ...
        if df is not None and not df.empty:
            ...
            if not r_all.empty:
                win = r_all.iloc[0]
                # [V8.1 Fix] IndexError 방지를 위한 안전한 격차 계산
                gap = 0
                if len(r_all) > 1:
...
st.rerun()
    ...
    if not sub_df.empty:
        ...
        fig_sub = px.bar(sub_df, x='기초지역', y='지지율', color='후보', text=sub_df['지지율'].apply(lambda x: f"{x:.1f}%"), 
                         ...
                         category_orders={'후보': fixed_sub_cand_order})
...
st.subheader("🗳️ 2025년 대선 vs 현재 실시간 판세 비교")
...
adj_minju = col_s1.slider("🔵 민주당 지지율 조정 (%p)", -15.0, 15.0, 0.0, 0.5)
adj_gukhim = col_s2.slider("🔴 국민의힘 지지율 조정 (%p)", -15.0, 15.0, 0.0, 0.5)
df_sim = df_prov.copy()
