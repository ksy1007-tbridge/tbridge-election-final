...
def final_visual_map_engine(df, title_text, highlight_region="", mode="normal", active_regions=None):
    ...
    for region, (col, row) in HEX_MAP.items():
        cx, cy = col * math.sqrt(3) + (row % 2 == 1) * (math.sqrt(3)/2), row * 1.5
        x_c, y_c = [], []
        for i in range(6):
            a = math.pi/6 + i*math.pi/3
            x_c.append(cx + math.cos(a)); y_c.append(cy + math.sin(a))
        ...
        fig.add_trace(go.Scatter(x=x_c, y=y_c, fill='toself', fillcolor=f_color, mode='lines', line=dict(color=l_color, width=l_width), hoverinfo='none'))
        fig.add_trace(go.Scatter(x=[cx], y=[cy], mode='text', text=[f"<b>{region}</b>"], textfont=dict(color=t_color, size=15), hoverinfo='none'))
...
if app_mode == "현행 판세 분석":
    ...
    if not sub_df.empty:
        ...
        # [정렬] 민주당 후보가 먼저 나오도록 강제
        sub_df['sort_pri'] = sub_df['정당'].apply(lambda x: 1 if '민주' in str(x) else (2 if '국힘' in str(x) else 99))
        sub_df = sub_df.sort_values(['기초지역', 'sort_pri'])
        
        st.plotly_chart(px.bar(sub_df, x='기초지역', y='지지율', color='후보', text=sub_df['지지율'].apply(lambda x: f"{x:.1f}%"), barmode='group', color_discrete_map=d_colors, category_orders={'후보': f_order}), use_container_width=True)
        st.write("### 📋 상세 데이터 (전체 포함)")
        st.dataframe(sub_df[['기초지역', '후보', '정당', '지지율']].sort_values(['기초지역', 'sort_pri']), hide_index=True, use_container_width=True)

elif app_mode == "2025 대선 비교 분석":
    df_prov = df_latest[df_latest['기초지역'] == '전체']
    # 2025 가상 데이터 정의
    past_df = pd.DataFrame([['서울','이재명','민주당',52],['충북','이재명','민주당',47],['충북','김문수','국힘',49]], columns=['지역','후보','정당','지지율'])
    c1, c2 = st.columns(2)
    with c1: st.plotly_chart(final_visual_map_engine(past_df, "2025 대선 결과"), use_container_width=True)
    ...
    c1, c2 = st.columns(2)
    m_adj = c1.slider("🔵 민주당 조정", -15.0, 15.0, 0.0)
    g_adj = c2.slider("🔴 국힘 조정", -15.0, 15.0, 0.0)
