...
def load_data():
    try:
        ...
        # [V9.1] 동일 날짜/지역/후보 데이터가 여러 개일 경우 평균값(Poll of Polls) 계산
...
def get_dynamic_color_map(df):
    ...
    if df is not None and not df.empty:
        for _, row in df.iterrows():
            ...
            elif '국힘' in party: c_map[cand] = COLOR_GUKHIM
...
def final_visual_map_engine(df, title_text, highlight_region="", mode="normal", active_regions=None):
    ...
    for region, (col, row) in HEX_MAP.items():
        ...
        for i in range(6):
            ...
            x_pts.append(cx + math.cos(a)); y_pts.append(cy + math.sin(a))
        x_pts.append(x_pts[0]); y_pts.append(y_pts[0])
        ...
        if region == highlight_region: f_color, l_color, l_width = SEL_FILL, SEL_LINE, 5
            ...
            if not r.empty:
                ...
                elif '국힘' in p: f_color = f'rgba(230, 30, 43, {alpha})'
...
with st.sidebar:
    ...
    st.divider()
    if st.button("🔄 실시간 데이터 새로고침", use_container_width=True): st.cache_data.clear(); st.rerun()
    st.divider(); st.caption("⚠️ 본 시스템의 인용 조사는 중앙선거여론조사심의위원회 홈페이지를 참조하시기 바랍니다.")
...
if app_mode == "현행 판세 분석":
    ...
    if not reg_latest.empty:
        ...
        fig_bar.update_layout(bargap=0.2, bargroupgap=0.0) # [V9.1] 막대 사이 공백 제거
    ...
    if not sub_df.empty:
        ...
        # [V9.1] 가독성 핵심 수정: bargroupgap=0 으로 설정하여 막대 밀착
