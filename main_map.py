...
if app_mode == "현행 판세 분석":
    ...
    # [추세 그래프 복구]
    region_all_history = df_all[(df_all['지역'] == sel_reg) & (df_all['기초지역'] == '전체')].sort_values('조사일자')
    if len(region_all_history['조사일자'].unique()) > 1:
        fig_trend = px.line(region_all_history, x='조사일자', y='지지율', color='후보', markers=True,
                            title=f"[{sel_reg}] 일자별 지지율 추세 변화",
                            color_discrete_map={'더불어민주당': '#004EA2', '국민의힘': '#E61E2B'})
    ...
    if not region_latest.empty:
        fig_bar = px.bar(region_latest, x='후보', y='지지율', color='정당', text=region_latest['지지율'].apply(lambda x: f"{x:.1f}%"),
                         title=f"[{sel_reg}] 최신 지지율 현황", color_discrete_map={'더불어민주당': '#004EA2', '국민의힘': '#E61E2B'})
    ...
    if is_valid:
        ...
        st.divider()
        # [추세 그래프 복구 - 광역 단위 추세를 먼저 보여줌]
        region_history = df_all[(df_all['지역'] == sel_reg) & (df_all['기초지역'] == '전체')].sort_values('조사일자')
        if len(region_history['조사일자'].unique()) > 1:
            st.markdown(f"### 📈 {sel_reg} 광역 지지율 추세 변화")
            fig_trend = px.line(region_history, x='조사일자', y='지지율', color='후보', markers=True, color_discrete_map={'더불어민주당': '#004EA2', '국민의힘': '#E61E2B'})
            st.plotly_chart(fig_trend, use_container_width=True)

        sub_df = df_latest[(df_latest['지역'] == sel_reg) & (df_latest['기초지역'] != '전체')]
        if not sub_df.empty:
            st.markdown(f"### 🚩 {sel_reg} 시군구별 상세 분석")
            fig_sub = px.bar(sub_df, x='기초지역', y='지지율', color='정당', text=sub_df['지지율'].apply(lambda x: f"{x:.1f}%"), barmode='group', color_discrete_map={'더불어민주당': '#004EA2', '국민의힘': '#E61E2B'})
