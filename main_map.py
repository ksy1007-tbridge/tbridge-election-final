        ...
        if region == highlight_region:
            ...
            if df is not None and not df.empty:
                ...
                if not r_all.empty:
                    ...
                    gap = 0
                    if len(r_all) > 1:
                        gap = win.get('지지율', 0) - r_all.iloc[1].get('지지율', 0)
...
if app_mode == "현행 판세 분석":
    ...
    if len(reg_hist['조사일자'].unique()) > 1:
        fig_trend = px.line(reg_hist, x='조사일자', y='지지율', color='후보', markers=True, title=f"[{sel_reg}] 지지율 추세 변화", color_discrete_map=dynamic_colors)
    ...
    if not reg_latest.empty:
        ...
        fig_bar = px.bar(reg_latest, x='후보', y='지지율', color='후보', text=reg_latest['지지율'].apply(lambda x: f"{x:.1f}%"), title=f"[{sel_reg}] 최신 지지율 현황", color_discrete_map=dynamic_colors, category_orders={'후보': fixed_cand_order})
    ...
    if is_valid:
        ...
        for i, reg in enumerate(all_regs):
            with cols[i % 6]:
                ...
                if st.button(f"{prefix}{reg}", key=f"m_btn_{reg}", use_container_width=True, type="primary" if st.session_state['selected_region'] == reg else "secondary"):
                    st.session_state['selected_region'] = reg
                    st.rerun() 
        ...
        if not sub_df.empty:
            ...
            fig_sub = px.bar(sub_df, x='기초지역', y='지지율', color='후보', text=sub_df['지지율'].apply(lambda x: f"{x:.1f}%"), barmode='group', color_discrete_map=dynamic_sub_colors, category_orders={'후보': fixed_sub_cand_order})
            st.plotly_chart(fig_sub, use_container_
