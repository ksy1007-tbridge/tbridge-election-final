...
# 2025 대선 데이터 (비교용)
past_data_list = [['서울', '이재명', '민주당', 52.0], ['서울', '김문수', '국힘', 45.0], ['경기', '이재명', '민주당', 54.0], ['경기', '김문수', '국힘', 43.0], ['충북', '김문수', '국힘', 49.0], ['충북', '이재명', '민주당', 47.0], ['전남', '이재명', '민주당', 86.0], ['경북', '김문수', '국힘', 75.0]]
...
def final_visual_map_engine(df, title_text, highlight_region="", mode="normal", active_regions=None):
    ...
    for region, (col, row) in HEX_MAP.items():
        ...
        fig.add_trace(go.Scatter(x=x_coords, y=y_coords, fill='toself', fillcolor=f_color, mode='lines', line=dict(color=l_color, width=l_width), name=region, hoverinfo='none'))
...
if app_mode == "현행 판세 분석":
    ...
    df_prov = df_latest[df_latest['기초지역'] == '전체']
    c1, c2 = st.columns(2)
    with c1: st.plotly_chart(final_visual_map_engine(df_2025, "🗳️ 2025년 대선 결과"), use_container_width=True)
