...
def get_order(df):
    """현재 데이터프레임(df)에 존재하는 후보들만 정해진 우선순위로 정렬해 리스트 반환"""
    ...
    # 현재 화면에 있는 후보들만 정렬된 리스트로 반환 (유령 슬롯 방지의 핵심)
...
def load_data():
    try:
        ...
        st.error(f"Data Error: {e}"); return None, None
...
if mode == "현행 판세":
    ...
    c = st.columns(6)
    for r in sorted(HEX_MAP.keys()):
        if c[list(sorted(HEX_MAP.keys())).index(r)%6].button(r, key=f"p{r}", use_container_width=True, type="primary" if sel==r else "secondary"):
    ...
    if not reg_lat.empty:
        # 현재 화면에 있는 후보들만 색상과 순서 추출
        colors = get_colors(reg_lat)
        order = get_order(reg_lat)
...
# [모드 2] 시군구 판세
    ...
    for r in sorted(HEX_MAP.keys()):
        p = "🔵 " if r in act else "⚪ "
        if c[list(sorted(HEX_MAP.keys())).index(r)%6].button(f"{p}{r}", key=f"m{r}", use_container_width=True, type="primary" if sel==r else "secondary"):
            st.session_state.sel_reg = r; st.rerun()
    ...
    if not sub.empty:
        # [V10.7 핵심] 지지율 0인 후보 제거 + 현재 지역 후보로만 리스트 한정
        sub = sub[sub['지지율'] > 0]
        colors = get_colors(sub)
        order = get_order(sub) # 이 함수가 현재 sub에 있는 후보만 반환함
        sorted_m = ['전체'] + sorted([m for m in sub['기초지역'].unique() if m != '전체'])
        
        # category_orders에 유령 후보가 포함되지 않도록 보장
        ...
        # 디자인 최적화: bargap은 적당히(0.15), bargroupgap은 완전히 밀착(0.0)
