...
def get_hexagon_path(col, row, radius=1):
    ...
    for i in range(6):
        ...
        x_pts.append(cx + radius*math.cos(a)); y_pts.append(cy + radius*math.sin(a))
...
if app_mode == "현행 판세 분석":
    ...
    # [V10.0] 현재 뷰에 지지율이 있는 후보만 필터링 (가독성 최적화)
    ...
    reg_latest = reg_latest[reg_latest['지지율'] > 0] # 0% 후보 제거
...
# [모드 2] 시군구 판세 분석
    ...
    if not sub_df.empty:
        # [V10.0 핵심] 지지율이 0보다 큰 실제 데이터만 남겨서 유령 좌석(Gap) 제거
        ...
        fig_sub.update_layout(bargap=0.2, bargroupgap=0.0) # 막대 밀착
